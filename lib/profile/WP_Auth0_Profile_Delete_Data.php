...
?>
		<table class="form-table">
			<tr>
				<th>
					<label><?php esc_html_e('Delete Auth0 Data', 'wp-auth0'); ?></label>
				</th>
				<td>
					<input type="button" id="auth0_delete_data" class="button button-secondary" value="<?php esc_html_e('Delete Auth0 Data', 'wp-auth0'); ?>" />
					<br><br>
					<a href="https://manage.auth0.com/#/users/<?php echo rawurlencode($auth0_user->sub); ?>" target="_blank">
						<?php esc_html_e('View in Auth0', 'wp-auth0'); ?>
					</a>
				</td>
			</tr>
		</table>
<?php
	...
	public function delete_user_data()
	{
		...
		if (empty($_POST['user_id'])) {
			wp_send_json_error(['error' => esc_html__('Empty user_id', 'wp-auth0')]);
		}

		$user_id = absint($_POST['user_id']);

		if (!current_user_can('edit_users')) {
			wp_send_json_error(['error' => esc_html__('Forbidden', 'wp-auth0')]);
		}
