...
class WP_Auth0_Import_Settings
{
	...
	public function import_settings()
	{
		...
		if (empty($settings_json)) {
			wp_safe_redirect(esc_url($this->make_error_url(esc_html__('No settings JSON entered.', 'wp-auth0'))));
			exit;
		}

		$settings = json_decode($settings_json, true);
		if (!$settings || !is_array($settings)) {
			wp_safe_redirect(esc_url($this->make_error_url(esc_html__('Settings JSON entered is not valid.', 'wp-auth0'))));
			exit;
		}
