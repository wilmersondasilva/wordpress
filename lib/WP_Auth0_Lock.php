...
class WP_Auth0_Lock
{
	...
	public static function render_back_to_lock()
	{
		...
		echo wp_kses(sprintf(
			...
			sanitize_text_field(sprintf(esc_html__('← Back to %s login', 'wp-auth0'), $title))
		), ['div' => ['id' => true], 'a' => ['href' => true]]);
	}
	...
	public static function render($canShowLegacyLogin = true, $specialSettings = [])
	{
		...
		wp_enqueue_script('wpa0_lock', $options->get_lock_url(), ['jquery'], WPA0_VERSION, true);
		...
		wp_localize_script(
			...
			[
				...
				'i18n'            => [
					'notReadyText'       => esc_html__('Auth0 is not configured', 'wp-auth0'),
					'cannotFindNodeText' => esc_html__('Auth0 cannot find node with id ', 'wp-auth0'),
