...
class WP_Auth0_LoginManager
{
	...
	public function init_auth0()
	{
		...
		if (!$state) {
			$this->die_on_login(esc_html__('Missing state', 'wp-auth0'));
		}

		if (!WP_Auth0_State_Handler::get_instance()->validate($state)) {
			$this->die_on_login(esc_html__('Invalid state', 'wp-auth0'));
		}
		...
		try {
			...
			$display_message = esc_html__('Invalid ID token', 'wp-auth0');
	...
	public function redirect_login()
	{
		...
		if (!$exchange_resp_body) {
			throw new WP_Auth0_LoginFlowValidationException(esc_html__('Error exchanging code', 'wp-auth0'));
		}
	...
	public function login_user($userinfo, $id_token = null, $access_token = null, $refresh_token = null)
	{
		...
		if (
			...
			if (empty($userinfo->email)) {
				throw new WP_Auth0_LoginFlowValidationException(
					esc_html__('This account does not have an email associated, as required by your site administrator.', 'wp-auth0')
				);
			}
		...
		if (!is_null($user)) {
			...
			try {
				...
				throw new WP_Auth0_LoginFlowValidationException(esc_html($e->getMessage()));
				...
				throw new WP_Auth0_LoginFlowValidationException(esc_html($msg));
	...
	private function do_login($user, $userinfo, $is_new, $id_token, $access_token, $refresh_token)
	{
		...
		try {
			...
			throw new WP_Auth0_BeforeLoginException(esc_html($e->getMessage()));
		}
	...
	protected function die_on_login($msg = '', $code = 0)
	{
		...
		$html = wp_kses(sprintf(
			...
			esc_html__('There was a problem with your log in', 'wp-auth0'),
			!empty($msg)
				? sanitize_text_field($msg)
				: esc_html__('Please see the site administrator', 'wp-auth0'),
			esc_html__('error code', 'wp-auth0'),
			esc_html($code ? sanitize_text_field($code) : esc_html__('unknown', 'wp-auth0')),
	...
	private function decode_id_token($id_token)
	{
		...
		if ('RS256' === $expectedAlg) {
			...
			throw new WP_Auth0_InvalidIdTokenException(esc_html('Signing algorithm of "' . $expectedAlg . '" is not supported.'));
		}
