...
class WP_Auth0_UsersRepo
{
	...
	public function create($userinfo, $token)
	{
		...
		if (is_object($wp_user) && $wp_user instanceof WP_User) {
			...
			if ($is_db_connection && !$email_verified) {
				throw new WP_Auth0_EmailNotVerifiedException(esc_html($userinfo), esc_html($token));
			}
			...
			if (!empty($current_auth0_id) && $auth0_sub !== $current_auth0_id) {
				throw new WP_Auth0_CouldNotCreateUserException(esc_html__('There is a user with the same email.', 'wp-auth0'));
			}
			...
			if (is_wp_error($user_id)) {
				throw new WP_Auth0_CouldNotCreateUserException(esc_html($user_id->get_error_message()));
			} elseif (-2 === $user_id) {
				// Registration rejected by wpa0_should_create_user filter in WP_Auth0_Users::create_user().
				throw new WP_Auth0_CouldNotCreateUserException(esc_html__('Registration rejected.', 'wp-auth0'));
			} elseif ($user_id < 0) {
	...
	public function find_auth0_user($id)
	{
		...
		if (empty($id)) {
			WP_Auth0_ErrorLog::insert_error(__METHOD__, esc_html__('Empty user id', 'wp-auth0'));
		...
		if ($users === []) {
			WP_Auth0_ErrorLog::insert_error(__METHOD__ . ' => get_users() ', esc_html__('User not found', 'wp-auth0'));
			return null;
		}
