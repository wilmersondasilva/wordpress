...
class WP_Auth0_Admin_Appearance extends WP_Auth0_Admin_Generic
{
	...
	public function init()
	{
		$options = [
			[
				'name'     => esc_html__('Passwordless Login', 'wp-auth0'),
				'opt'      => 'passwordless_enabled',
				'id'       => 'wpa0_passwordless_enabled',
				'function' => 'render_passwordless_enabled',
			],
			[
				'name'     => esc_html__('Icon URL', 'wp-auth0'),
				'opt'      => 'icon_url',
				'id'       => 'wpa0_icon_url',
				'function' => 'render_icon_url',
			],
			[
				'name'     => esc_html__('Form Title', 'wp-auth0'),
				'opt'      => 'form_title',
				'id'       => 'wpa0_form_title',
				'function' => 'render_form_title',
			],
			[
				'name'     => esc_html__('Enable Gravatar Integration', 'wp-auth0'),
				'opt'      => 'gravatar',
				'id'       => 'wpa0_gravatar',
				'function' => 'render_gravatar',
			],
			[
				'name'     => esc_html__('Login Name Style', 'wp-auth0'),
				'opt'      => 'username_style',
				'id'       => 'wpa0_username_style',
				'function' => 'render_username_style',
			],
			[
				'name'     => esc_html__('Primary Color', 'wp-auth0'),
				'opt'      => 'primary_color',
				'id'       => 'wpa0_primary_color',
				'function' => 'render_primary_color',
			],
			[
				'name'     => esc_html__('Extra Settings', 'wp-auth0'),
				'opt'      => 'extra_conf',
				'id'       => 'wpa0_extra_conf',
				'function' => 'render_extra_conf',
			],
			[
				'name'     => esc_html__('Use Custom Lock JS URL', 'wp-auth0'),
				'opt'      => 'custom_cdn_url',
				'id'       => 'wpa0_custom_cdn_url',
				'function' => 'render_custom_cdn_url',
			],
			[
				'name'     => esc_html__('Custom Lock JS URL', 'wp-auth0'),
				'opt'      => 'cdn_url',
				'id'       => 'wpa0_cdn_url',
				'function' => 'render_cdn_url',
			],
			[
				'name'     => esc_html__('Connections to Show', 'wp-auth0'),
	...
	public function render_custom_cdn_url($args = [])
	{
		...
		$this->render_field_description(esc_html__('Use a custom Lock CDN URL instead of the default. ', 'wp-auth0'));

		if (!$this->options->get($args['opt_name'])) {
			$this->render_field_description(
				esc_html__('Currently using:', 'wp-auth0') .
	...
	public function render_cdn_url($args = [])
	{
		...
		$this->render_field_description(
			esc_html__('This should point to the latest Lock JS available in the CDN and rarely needs to change', 'wp-auth0')
		);
	}
	...
	public function render_passwordless_enabled($args = [])
	{
		...
		$this->render_field_description(
			esc_html__('Turn on Passwordless login (email or SMS) in the Auth0 form. ', 'wp-auth0') .
				esc_html__('Passwordless connections are managed in the ', 'wp-auth0') .
				$this->get_dashboard_link('connections/passwordless') .
				esc_html__(' and at least one must be active and enabled on this Application for this to work. ', 'wp-auth0') .
				esc_html__('Username/password login is not enabled when Passwordless is on', 'wp-auth0')
		);
	}
	...
	public function render_connections($args = [])
	{
		...
		$this->render_field_description(
			esc_html__('Specify which Social, Database, or Passwordless connections to display in the Auth0 form. ', 'wp-auth0') .
				esc_html__('If this is empty, all enabled connections for this Application will be shown. ', 'wp-auth0') .
				esc_html__('Separate multiple connection names with a comma. ', 'wp-auth0') .
				sprintf(
					// translators: HTML link to the Auth0 dashboard.
					esc_html__('Connections listed here must already be active in your %s', 'wp-auth0'),
					$this->get_dashboard_link('connections/social')
				) .
				esc_html__(' and enabled for this Application. ', 'wp-auth0') .
				esc_html__('Click on a Connection and use the "Name" value in this field', 'wp-auth0')
		);
	}
	...
	public function render_icon_url($args = [])
	{
		...
		echo wp_kses(sprintf(' <a id="wpa0_choose_icon" class="button button-secondary">%s</a>', esc_html__('Choose Icon', 'wp-auth0')), ['a' => ['id' => true, 'class' => true]]);
		$this->render_field_description(
			esc_html__('Icon above the title on the Auth0 login form. ', 'wp-auth0') .
				esc_html__('This image works best as a PNG with a transparent background less than 120px tall', 'wp-auth0')
		);
	}
	...
	{
		$this->render_text_field($args['label_for'], $args['opt_name']);
		$this->render_field_description(esc_html__('Title used on the Auth0 login form', 'wp-auth0'));
	}
	...
	public function render_gravatar($args = [])
	{
		$this->render_switch($args['label_for'], $args['opt_name']);
		$this->render_field_description(
			esc_html__('Automatically display an avatar (from Gravatar) on the Auth0 login form', 'wp-auth0')
		);
	}
	...
	public function render_username_style($args = [])
	{
		...
		$this->render_field_description(
			esc_html__('To allow the user to use either email or username to login, leave this as "Auto." ', 'wp-auth0') .
				esc_html__('Only database connections that require a username will allow username logins', 'wp-auth0')
		);
	}
	...
	public function render_primary_color($args = [])
	{
		$this->render_text_field($args['label_for'], $args['opt_name']);
		$this->render_field_description(
			esc_html__('Primary color for the Auth0 login form in hex format. ', 'wp-auth0') .
				$this->get_docs_link(
					'libraries/lock/v11/configuration#primarycolor-string-',
					esc_html__('More information on this settings', 'wp-auth0')
				)
		);
	}
	...
	public function render_extra_conf($args = [])
	{
		...
		$this->render_field_description(
			esc_html__('Valid JSON for Lock options configuration; will override all options set elsewhere. ', 'wp-auth0') .
	...
	public function basic_validation(array $input)
	{
		...
		if (!empty($input['icon_url']) && !filter_var($input['icon_url'], FILTER_VALIDATE_URL)) {
			...
			self::add_validation_error(esc_html__('The Icon URL used is not valid.', 'wp-auth0'));
		}
		...
		if (!empty($input['extra_conf']) && !json_decode($input['extra_conf'])) {
			...
			$error               = esc_html__('The Extra Settings parameter should be a valid JSON object.', 'wp-auth0');
		...
		if ($input['custom_cdn_url'] && !filter_var($input['cdn_url'], FILTER_VALIDATE_URL)) {
			...
			self::add_validation_error(esc_html__('The Custom Lock JS URL used is not valid.', 'wp-auth0'));
		}
