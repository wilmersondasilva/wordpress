...
class WP_Auth0_Admin_Features extends WP_Auth0_Admin_Generic
{
	...
	public function render_singlelogout($args = [])
	{
		...
		$this->render_field_description(
			esc_html__('Turning this on will log users out of Auth0 when they log out of WordPress.', 'wp-auth0')
		);
	}
	...
	public function render_auto_login($args = [])
	{
		...
		$this->render_field_description(
			esc_html__('Use the Universal Login Page (ULP) for authentication and SSO. ', 'wp-auth0') .
				wp_kses(__('When turned on, <code>wp-login.php</code> will redirect to the hosted login page. ', 'wp-auth0'), ['code' => []]) .
				wp_kses(__('When turned off, <code>wp-login.php</code> will show an embedded login form. ', 'wp-auth0'), ['code' => []]) .
				$this->get_docs_link('guides/login/universal-vs-embedded', esc_html__('More on ULP vs embedded here', 'wp-auth0'))
		);
	}
	...
	{
		$this->render_text_field($args['label_for'], $args['opt_name'], 'text', '', '', 'wpa0_auto_login_options');
		$this->render_field_description(
			esc_html__('Optional. Here you can specify additional parameters to pass to the the Universal Login Page (ULP) during authentication. ', 'wp-auth0') .
				wp_kses(__('For example, you can specify <code>screen_hint=signup</code> or <code>prompt=login</code> parameters here. ', 'wp-auth0'), ['code' => []]) .
				$this->get_docs_link('docs/login/universal-login/new-experience', esc_html__('Learn more about available ULP parameters here', 'wp-auth0'))
		);
	}
	...
	public function render_auto_login_method($args = [])
	{
		$this->render_text_field($args['label_for'], $args['opt_name'], 'text', '', '', 'wpa0_auto_login_options');
		$this->render_field_description(
			esc_html__('Enter a name here to automatically use a single, specific connection to login . ', 'wp-auth0') .
				wp_kses(sprintf(
					// translators: Placeholder is an HTML link to the Auth0 dashboard.
					esc_html__('Find the method name to use under Connections > [Connection Type] in your %s. ', 'wp-auth0'),
					$this->get_dashboard_link()
				), ['a' => ['href' => true, 'target' => true]]) .
				esc_html__('Click the expand icon and use the value in the "Name" field (like "google-oauth2")', 'wp-auth0')
		);
	}
	...
	public function render_override_wp_avatars($args = [])
	{
		$this->render_switch($args['label_for'], $args['opt_name']);
		$this->render_field_description(
			esc_html__('Overrides the WordPress avatar with the Auth0 profile avatar', 'wp-auth0')
		);
	}
